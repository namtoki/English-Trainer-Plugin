...
収集した単語を **ファイル別にグループ化** し、複数ファイルから満遍なくピックアップしてください:

1. Grep 結果からファイルパスごとに単語をグループ化
2. 各ファイルから **ラウンドロビン方式** で1語ずつ選択
   - 例: 5問の場合、5つの異なるファイルから1語ずつ
   - ファイル数が足りない場合は、2周目以降も異なるファイルを優先
3. 同一ファイルからの連続選択を避ける
